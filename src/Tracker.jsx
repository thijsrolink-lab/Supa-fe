...
import { Square, CheckSquare, Stamp, Ruler, Weight, Plus, ChevronLeft, ChevronRight, Users, Settings, LogOut } from "lucide-react";
...
export default function Tracker({ child, siblingNames, partnerName, childOptions, onSelectChild, onEditFamily, onLogout }) {
  ...
  const addEntry = async () => {
    ...
    const row = {
      ...
      length: lengthInput ? parseFloat(lengthInput) : null,
    };
    ...
    if (error) {
      ...
      setEntries(prev => [...prev.filter(e => e.week !== currentWeek), row].sort((a, b) => a.week - b.week));
      setWeightInput("");
      setLengthInput("");
      setSaveMsg("Genoteerd.");
    }
    ...
    <div style={styles.wrap}>
      <div style={styles.container}>
        ...
        <div style={styles.page}>
          ...
          <div style={styles.sectionLabel}>Meting — week {currentWeek}</div>
            ...
            <div style={styles.chartBox}>
              ...
              <ResponsiveContainer width="100%" height={240}>
                <LineChart data={weightData} margin={{ top: 10, right: 8, left: -8, bottom: 0 }}>
                  ...
                  <Line type="monotone" dataKey="gemeten" name={child.name} stroke="#2F6F62" strokeWidth={3} dot={{ r: 4 }} connectNulls />
                </LineChart>
              </ResponsiveContainer>
            </div>
            <div style={styles.chartBox}>
              ...
              <ResponsiveContainer width="100%" height={240}>
                <LineChart data={lengthData} margin={{ top: 10, right: 8, left: -8, bottom: 0 }}>
                  ...
                  <Line type="monotone" dataKey="gemiddeld" name="gemiddelde" stroke="#8A8368" strokeWidth={1.5} strokeDasharray="4 3" dot={false} />
                  <Line type="monotone" dataKey="onder" name="ondergrens" stroke="#6B6550" strokeWidth={1.5} strokeDasharray="4 3" dot={false} />
                  <Line type="monotone" dataKey="gemeten" name={child.name} stroke="#B0483D" strokeWidth={3} dot={{ r: 4 }} connectNulls />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
